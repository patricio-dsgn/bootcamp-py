...
print('Modifique todos los ID. Agregue la siguiente cadena de caracteres:')
# “_piloto” al final de cada ID. Imprima en pantalla los nuevos ID.
print('\t\"_piloto\" al final de cada ID. Imprima en pantalla los nuevos ID.')
keys = list(clientes.keys())
for key in keys:
    clientes[str(key)+'_piloto'] = clientes[key]
    del clientes[key]
print('\tresultado')
print(clientes.keys())

# Elimine los últimos cuatro ID_clientes en el listado.
print('\tElimine los últimos cuatro ID_clientes en el listado.')
for key in list(clientes.keys())[-5:-1]:
    del clientes[key]
print('\tresultado')
print(clientes.keys())